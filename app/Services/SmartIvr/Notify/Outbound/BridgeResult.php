...
use App\Services\SmartIvr\Events\Notify\Outbound\BridgedEvent;
...
class BridgeResult extends Notify
{
    public function handle()
    {
        ...
        if ($this->getErrorCode() == 0) {
            ...
            $payload = new Hangup();
            $payload->usermsg('bridge_success');
        } else {
            //转接失败，继续AI通话
            $handle->setBridgeStatus(BridgeStatus::FAILED);
            $payload = $handle->bridgeFailedFlow()->getPayload($this);
            if (empty($payload)) {
                $payload = new Hangup();
                $payload->usermsg('bridge_failed');
            }
        }
